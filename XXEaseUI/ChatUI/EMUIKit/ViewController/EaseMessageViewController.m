...
-(instancetype)initWithConversationChatter:(NSString *)conversationChatter nickname:(NSString *)nickname avatar:(NSString *)avatar ownNickname:(NSString *)ownNickname ownAvatar:(NSString *)ownAvatar{
    self = [self initWithConversationChatter:conversationChatter conversationType:EMConversationTypeChat];
    self.nickname = ownNickname;
    self.avatar = ownAvatar;
    self.title = nickname;
    [EMConversation saveConversationWithConversationId:conversationChatter nickname:nickname avatar:avatar];
    return self;
}
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    [[EaseBaseMessageCell appearance] setSendBubbleBackgroundImage:[[UIImage imageNamed:@"EaseUIResource.bundle/newAdd/into-blue_bj-@2x"] stretchableImageWithLeftCapWidth:5 topCapHeight:35]];
    [[EaseBaseMessageCell appearance] setRecvBubbleBackgroundImage:[[UIImage imageNamed:@"EaseUIResource.bundle/newAdd/into-white_bj@2x"] stretchableImageWithLeftCapWidth:35 topCapHeight:35]];

    [[EaseBaseMessageCell appearance] setSendMessageVoiceAnimationImages:@[[UIImage imageNamed:@"EaseUIResource.bundle/newAdd/yuyin-blue_icon"], [UIImage imageNamed:@"EaseUIResource.bundle/newAdd/yuyin1-blue_icon-"], [UIImage imageNamed:@"EaseUIResource.bundle/newAdd/yuyin2-blue_icon"]]];
    [[EaseBaseMessageCell appearance] setRecvMessageVoiceAnimationImages:@[[UIImage imageNamed:@"EaseUIResource.bundle/newAdd/yuyin-grey_icon"],[UIImage imageNamed:@"EaseUIResource.bundle/newAdd/yuyin1-grey_icon"], [UIImage imageNamed:@"EaseUIResource.bundle/newAdd/yuyin2-grey_icon"]]];
...
-(void)setHasNewMessages:(bool)hasNewMessages{
    ...
    self.rightItem.selected = _hasNewMessages;
}

-(void)setRightItemImage:(UIImage *)image selectImage:(UIImage *)selectImage touch:(TouchBlock)block{
    self.rightItem = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.rightItem setImage:image forState:UIControlStateNormal];
    [self.rightItem setImage:selectImage forState:UIControlStateSelected];
    [self.rightItem addTarget:self action:@selector(clickRightItem) forControlEvents:UIControlEventTouchUpInside];
    self.rightItem.frame = CGRectMake(0, 0, 30, 30);
    UIBarButtonItem * item = [[UIBarButtonItem alloc] initWithCustomView:self.rightItem];
    self.navigationItem.rightBarButtonItem = item;
    self.touchRightItemBlock = block;
    
}
-(void)clickRightItem{
    if (self.touchRightItemBlock){
        self.touchRightItemBlock();
    }
}
        ...
        if (isPrepare) {
            ...
            [[EMCDDeviceManager sharedInstance] asyncPlayingWithPath:model.fileLocalPath completion:^(NSError *error) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [[EMCDDeviceManager sharedInstance] disableProximitySensor];
                    [weakSelf autoReadNextVoiceMessages:model];
                });
            }];
        }
        else{
...
-(void)autoReadNextVoiceMessages:(id<IMessageModel>)model{
    ...
    for (NSUInteger i = index; i < self.dataArray.count; i++) {
        id<IMessageModel> nextModel = self.dataArray[i];
        if ([nextModel isKindOfClass:[NSString class]]){
            continue;
        }
        if ([nextModel.message body].type != EMMessageBodyTypeVoice || [nextModel.message direction] == EMMessageDirectionSend || [nextModel.message isRead] ) {
            break;
        }
        [self _audioMessageCellSelected:nextModel];
        break;
...
-(void)readImages:(id<IMessageModel>)selectModel{
    NSMutableArray * images = [NSMutableArray array];
    NSUInteger index = 0;
    for (id<IMessageModel>model in self.dataArray) {
        ...
        if ([imageBody type] == EMMessageBodyTypeImage) {
            ...
            [images addObject:[NSURL URLWithString:imageBody.thumbnailRemotePath]];
            }
            if ([model.message.messageId isEqualToString:[selectModel.message messageId]]){
                index = images.count > 0 ? images.count - 1 : 0;
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if (!(bool)[model isSender]&& self.title == nil){
            NSString * name = [[model message] valueForKeyPath:USER_NickName];
            if (name){
                self.title = name;
            }
        }
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(isEmotionMessageFormessageViewController:messageModel:)]) {
            ...
            if (flag) {
                ...
                sendCell.sendAvatarUrlPath = self.avatar;
        ...
        sendCell.sendAvatarUrlPath = self.avatar;
//        [model setAvatarImage:[UIImage imageNamed:@"pen_icon"]];
        sendCell.model = model;
        return sendCell;
    }
}
...
- (void)didFinishRecoingVoiceAction:(UIView *)recordView
{
    ...
    [[EMCDDeviceManager sharedInstance] asyncStopRecordingWithCompletion:^(NSString *recordPath, NSInteger aDuration, NSError *error) {
        if (!error) {
            ...
            [weakSelf showHudInView:self.view hint:@"录音时间较短"];
...
- (void)_sendMessage:(EMMessage *)message
{
    ...
    message.ext = @{@"imUserInfo":@{@"nickname":self.nickname,@"avatar":self.avatar}};
