...
//     __weak typeof(view) weakView = view;
    ...
    UIColor * color = [UIColor colorWithRed:33/255.0 green:33/255.0 blue:33/255.0 alpha:1.0];
...
+ (MBProgressHUD *)MB_ShowMessage:(UIView *)view message:(NSString *)message{
    if (view) {
        MBProgressHUD * hud = [self HUDForView:view];
        if (!hud) {
        ...
        CGFloat h =  [self labelAutoCalculateRectWith:message Font:[UIFont systemFontOfSize:15] MaxSize:CGSizeMake(view.bounds.size.width - 50, view.bounds.size.height - 90 - 64 - 2 * 49)].height + 90;
        UIView * backView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, view.bounds.size.width - 40, h)];
        
        UILabel * titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, backView.frame.size.width, 40)];
        titleLabel.textAlignment = NSTextAlignmentCenter;
        titleLabel.text = @"提醒";
        titleLabel.font = [UIFont boldSystemFontOfSize:18];
        [backView addSubview:titleLabel];
        
        UILabel * desLabel = [[UILabel alloc] initWithFrame:CGRectMake(5, CGRectGetMaxY(titleLabel.frame), backView.bounds.size.width - 10, h - 90)];
        ...
        button.frame = CGRectMake(5, CGRectGetMaxY(desLabel.frame) + 5, backView.bounds.size.width - 10, 40);
        button.backgroundColor = [UIColor colorWithRed:226/255.0 green:13/255.0 blue:31/255.0 alpha:1.0];
        [button setTitle:@"确定" forState:UIControlStateNormal];
        button.layer.masksToBounds = YES;
        button.layer.cornerRadius = 5;
        [backView addSubview:button];
        [button addTarget:hud action:@selector(clickButton:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)completion:(MBProgressHUDCompletionBlock)completion{
