...
-(void)asyncPlayingUrlPath:(NSString *)aFilePath completion:(void (^)(NSError *))completon{
    ...
    [EMAudioPlayerUtil asyncPlayingWithUrlPath:aFilePath completion:^(NSError *error) {
        [self setupAudioSessionCategory:EM_DEFAULT
                               isActive:NO];
        if (completon) {
            completon(error);
