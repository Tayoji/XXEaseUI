...
+(void)asyncPlayingWithUrlPath:(NSString *)aFilePath completion:(void (^)(NSError *))completon{
    [[EMAudioPlayerUtil sharedInstance] asyncPlayingWithUrlPath:aFilePath completion:completon];
}
...
-(void)asyncPlayingWithUrlPath:(NSString *)aFilePath
completion:(void(^)(NSError *error))completon{
    playFinish = completon;
     NSError *error = nil;
    NSURL *url = [[NSURL alloc] initWithString:aFilePath];
