            ...
            case EMMessageBodyTypeVoice:
            {
                if (_bubbleView.voiceImageView) {
                    if ([self.sendMessageVoiceAnimationImages count] > 0 && [self.recvMessageVoiceAnimationImages count] > 0) {
                        ...
                        self.bubbleView.voiceImageView.image = self.model.isSender ?[UIImage imageNamed:@"EaseUIResource.bundle/newAdd/yuyin-blue_icon"]: [UIImage imageNamed:@"EaseUIResource.bundle/newAdd/yuyin-grey_icon"];
                    }
                }
...
//            height += (retSize.height > 20 ? retSize.height : 20) + 10;
        ...
        case EMMessageBodyTypeFile:
        {
            ...
            if ([NSString instancesRespondToSelector:@selector(boundingRectWithSize:options:attributes:context:)]) {
                ...
                //[text sizeWithFont:font constrainedToSize:CGSizeMake(bubbleMaxWidth, CGFLOAT_MAX) lineBreakMode:NSLineBreakByCharWrapping];
