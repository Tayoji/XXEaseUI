...
- (void)layoutSubviews
{
    ...
    switch (self.model.bodyType) {
        ...
        {
            [self removeConstraint:self.bubbleWithImageConstraint];
            CGFloat mu = ([self.model mediaDuration] > 60 ? 60 : [self.model mediaDuration])/60 * (self.bubbleMaxWidth - 50);
            self.bubbleWithImageConstraint = [NSLayoutConstraint constraintWithItem:self.bubbleView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:50 + mu];
            [self addConstraint:self.bubbleWithImageConstraint];
        }
            break;
...
{
    if (model.isSender) {
        [self configureSendLayoutConstraints:model];
    } else {
...
- (void)configureSendLayoutConstraints:(id<IMessageModel>)model
{
    ...
    if ([model bodyType] == EMMessageBodyTypeVoice){
        //status button
        [self addConstraint:[NSLayoutConstraint constraintWithItem:self.statusButton attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.bubbleView.voiceDurationLabel attribute:NSLayoutAttributeLeft multiplier:1.0 constant:-15]];
        
        //activity
        [self addConstraint:[NSLayoutConstraint constraintWithItem:self.activity attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.bubbleView.voiceDurationLabel attribute:NSLayoutAttributeLeft multiplier:1.0 constant:-15]];
        
        //hasRead
        [self addConstraint:[NSLayoutConstraint constraintWithItem:self.hasRead attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.bubbleView.voiceDurationLabel attribute:NSLayoutAttributeLeft multiplier:1.0 constant:0]];
...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if ((bool)[model isSender]){
        [self.avatarView sd_setImageWithURL:[NSURL URLWithString:self.sendAvatarUrlPath] placeholderImage:model.avatarImage];
    }else{
        NSString * avatarURLPath = [[model message] valueForKeyPath:USER_Avatar];
         [self.avatarView sd_setImageWithURL:[NSURL URLWithString:avatarURLPath] placeholderImage:model.avatarImage];
...
+ (CGFloat)cellHeightWithModel:(id<IMessageModel>)model
{
    ...
    CGFloat minHeight = cell.avatarSize + EaseMessageCellPadding * 2 - 10;
    CGFloat height = cell.messageNameHeight;
    if ([UIDevice currentDevice].systemVersion.floatValue == 7.0) {
        height = 15;
    }
    height += - EaseMessageCellPadding + [EaseMessageCell cellHeightWithModel:model] - 5;
